...
void EnvelopeGenerator::CalculateMultiplier(
    float start_level, float end_level, unsigned long long length_in_samples) {
  multiplier_ = 1.0f + (std::log(end_level) - std::log(start_level)) /
                           ((float)length_in_samples);
}

void EnvelopeGenerator::EnterStage(EnvelopeStage new_stage,
                                   const GeneratorPatch::Envelope &envelope) {
  const float stage_values[]{0.0f, envelope.A_R, envelope.D_R, envelope.S_L,
                             envelope.R_R};
  stage_ = new_stage;
  ...
  switch (new_stage) {
    ...
    case ENVELOPE_STAGE_ATTACK:
      ...
      CalculateMultiplier(current_level_, 1.0f, next_stage_sample_index_);
      break;
    ...
    case ENVELOPE_STAGE_RELEASE:
      ...
      CalculateMultiplier(current_level_, minimum_level_,
                          next_stage_sample_index_);
      break;
    default:
      break;
